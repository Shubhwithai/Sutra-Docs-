...
1. Open the notebook in Colab using the provided link: [Open in Colab](https://colab.research.google.com/drive/18BffQ6e0xrIpF97jRniLci2mAEvxHSl7?usp=sharing).
...
### In Google Colab
1. Open the notebook via the Colab link: [Open in Colab](https://colab.research.google.com/drive/18BffQ6e0xrIpF97jRniLci2mAEvxHSl7?usp=sharing).
2. Set `SUTRA_API_KEY` and `OPENAI_API_KEY` in Colab Secrets.
3. Upload a PDF to Colab (e.g., `/content/your_pdf.pdf`) and update the path in Step 3.
4. Run the cells sequentially to:
   - Install dependencies.
   - Set up API keys.
   - Load and process the PDF.
   - Create the RAG chain.
   - Test with questions (e.g., `ask_question("what is transformer", language="Hindi")`).
5. View the responses in the Colab output.
...
1. Install dependencies:
   ...
   pip install langchain langchain-openai langchain-community faiss-cpu pypdf python-docx requests jupyter python-dotenv
   ```
2. Set up the `.env` file with `SUTRA_API_KEY` and `OPENAI_API_KEY`.
3. Save the code as `multilingual_chat_with_pdf.ipynb`.
4. Place your PDF in the project directory and update the path in Step 3 (e.g., `./your_pdf.pdf`).
5. Run Jupyter Notebook:
   ```bash
   jupyter notebook
   ```
6. Open and run the notebook cells in sequence.
...
- [Complete Notebook Source](https://github.com/sutra-dev/sutra-cookbook/blob/main/notebooks/multilingual_chat_with_pdf.ipynb)
- [Colab Notebook Link](https://colab.research.google.com/drive/18BffQ6e0xrIpF97jRniLci2mAEvxHSl7?usp=sharing)
