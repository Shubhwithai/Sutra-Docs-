...
export default function ArticleLayout() {
  ...
  function getDefaultValue() {
    // First check if the current path is in the contents array
    let defaultValue = contents.findIndex((item) =>
      item.list.some((listItem) => listItem.href === pathname)
    );
    
    // If not found in contents, check if it's in cookbooks
    if (defaultValue === -1 && pathname.includes('cookbooks')) {
      defaultValue = contents.findIndex(item => item.title === 'Cookbooks');
    }
    
    // If not found and it's a Features path, find the Features section
    if (defaultValue === -1 && pathname.includes('Features')) {
      defaultValue = contents.findIndex(item => item.title === 'Features');
    }
    
    // If not found and it's a multilingual path, find the Multilingual section
    if (defaultValue === -1 && pathname.includes('multilingual')) {
      defaultValue = contents.findIndex(item => item.title === 'Multilingual');
    }
    
    // If not found and it's a Popular-Usecases path, find the Popular Use Cases section
    if (defaultValue === -1 && pathname.includes('Popular-Usecases')) {
  ...
  useEffect(() => {
    ...
    if (pathname.includes("cookbooks")) {
      ...
      // Make sure the Features section is expanded when viewing feature pages
      const featuresIndex = contents.findIndex(item => item.title === "Features");
      if (featuresIndex !== -1) {
        setCurrentOpen(featuresIndex);
        shouldSetDefaultValue = false; // Don't override the currentOpen value
      }
    } else if (pathname.includes("Multilingual")) {
      // Make sure the Multilingual section is expanded when viewing multilingual pages
      const multilingualIndex = contents.findIndex(item => item.title === "Multilingual");
      if (multilingualIndex !== -1) {
        setCurrentOpen(multilingualIndex);
        shouldSetDefaultValue = false; // Don't override the currentOpen value
      }
    } else if (pathname.includes("Popular-Usecases")) {
      // Make sure the Popular Use Cases section is expanded when viewing use case pages
      const useCasesIndex = contents.findIndex(item => item.title === "Popular Use Cases");
      if (useCasesIndex !== -1) {
        setCurrentOpen(useCasesIndex);
        shouldSetDefaultValue = false; // Don't override the currentOpen value
      }
    }
    ...
    // Only set the default value if we haven't already set currentOpen for special sections
